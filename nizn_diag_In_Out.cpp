...
{
	int nizn_diag::InData(ifstream& ifst) // ¬вод параметров диагональной матрицы
	{
		ifst >> size;
	...
	void nizn_diag::InDataN(ifstream& ifst) // ¬вод параметров диагональной матрицы
	{
		ifst >> size;
	...
	void nizn_diag::InDataNN(ifstream& ifst) // ¬вод параметров диагональной матрицы
	{
		ifst >> size;
	...
	void nizn_diag::OutStrokiN(ofstream& ofst) // ¬ывод параметров диагональной матрицы
	{
		ofst << "It is Nizn_Diag: Size = " << size << endl;
	...
	// мультиметод 
	void nizn_diag::Multimethod(matr* other, ofstream& ofst)
	{
		other->MMNiznDiag(ofst);
	}

	void nizn_diag::MMSquare(ofstream& ofst)
	{
		ofst << "Square and Nizn Diag" << endl;
	}

	void nizn_diag::MMDiag(ofstream& ofst)
	{
		ofst << "Diag and Nizn Diag" << endl;
	}

	void nizn_diag::MMNiznDiag(ofstream& ofst)
	{
		ofst << "Nizn Diag and Nizn Diag" << endl;
	}
